...
# Copy device plugins
for f in "${BUILT_PRODUCTS_DIR}"/*.plugin; do
  plugin=$(basename "$f")
  ...
  if [ "$EXPANDED_CODE_SIGN_IDENTITY" != "-" ]; then
    ...
    for framework_path in "${f}"/Frameworks/*.framework; do
      ...
      /usr/bin/codesign --force --sign ${EXPANDED_CODE_SIGN_IDENTITY} --timestamp=none --preserve-metadata=identifier,entitlements,flags "${BUILT_PRODUCTS_DIR}/${FRAMEWORKS_
